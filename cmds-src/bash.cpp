// Linuxify Bash Interpreter - Entry Point
// A shell script interpreter with bash-like syntax
...
void printUsage() {
    ...
    std::cout << "Linuxify Bash Interpreter v1.0\n";
    SetConsoleTextAttribute(h, FOREGROUND_BLUE | FOREGROUND_INTENSITY);
    std::cout << "A native shell script interpreter for Windows\n\n";
    SetConsoleTextAttribute(h, FOREGROUND_RED | FOREGROUND_GREEN | FOREGROUND_BLUE);
    
    std::cout << "Usage:\n";
    std::cout << "  bash <script.sh>        Run a shell script\n";
    std::cout << "  bash -c \"<command>\"     Run a command\n";
    std::cout << "  bash                    Interactive mode\n";
    std::cout << "  bash --debug <script>   Run with debug output\n\n";
...
int main(int argc, char* argv[]) {
    ...
    if (arg1 == "-c") {
        if (argc > 2) {
            ...
            std::cerr << "bash: -c requires an argument\n";
            return 1;
        }
    }
