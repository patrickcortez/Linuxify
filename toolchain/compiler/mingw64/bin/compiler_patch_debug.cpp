// Compiler Patch - Enforce No Comment Policy [DEBUG]
#include <iostream>
#include <string>
#include <vector>
#include <fstream>
...
bool hasComments(const std::string& filePath) {
    ...
    if (!file.is_open()) {
        std::cerr << "[DEBUG] Failed to open: " << filePath << std::endl;
        return false; 
    }
    ...
    while (file.get(c)) {
        if (c == '"' && !inChar) {
            ...
            int next = file.peek();
            if (next == '/' || next == '*') {
                std::cerr << "[DEBUG] Comment found in " << filePath << std::endl;
                return true;
            }
        }
    }
...
int main(int argc, char* argv[]) {
    std::cerr << "[DEBUG] Patcher is running. Arg count: " << argc << std::endl;
